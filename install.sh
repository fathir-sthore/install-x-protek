...
check_token() {
  echo -e "                                                       "
  echo -e "${BLUE}[+] =============================================== [+]${NC}"
  echo -e "${BLUE}[+]               LICENSE fathir sthore            [+]${NC}"
  echo -e "${BLUE}[+] =============================================== [+]${NC}"
  echo -e "                                                       "
  echo -e "${YELLOW}MASUKKAN AKSES TOKEN: ${NC}"
  read -r USER_TOKEN

  if [ "$USER_TOKEN" = "fathir" ]; then
    echo -e "${GREEN}AKSES BERHASIL${NC}}"
  else
    echo -e "${GREEN}Token Salah! Beli Kode Token Di fathir sthore${NC}"
    echo -e "${YELLOW}TELEGRAM : @fathir${NC}"
    echo -e "${YELLOW}WHATSAPP : +62882003493812${NC}"
    echo -e "${YELLOW}HARGA TOKEN : 10K FREE UPDATE JIKA ADA TOKEN BARU${NC}"
    echo -e "${YELLOW}© fathir sthore${NC}"
    exit 1
  fi
  clear
}
...
install_theme() {
  while true; do
    ...
    case "$SELECT_THEME" in
      1)
        THEME_URL=$(echo -e "https://github.com/fathir-sthore/install-x-protek/raw/refs/heads/main/stellar.zip")  
        break
        ;;
      2)
        THEME_URL=$(echo -e "https://github.com/fathir-sthore/install-x-protek/raw/refs/heads/main/billing.zip")
        break
        ;;
      3)
...
# Unduh file tema
wget -O /root/stellar.zip https://github.com/fathir-sthore/install-x-protek/raw/refs/heads/main/stellar.zip
...
while true; do
  ...
  echo -e "${RED}ggg:     gg.     -   ,ggg       • Telegram : @fathir      ${NC}"
  echo -e "${RED} ggg:     ggg._    _,ggg        • Whatsapp  : +62882003493812  ${NC}"
